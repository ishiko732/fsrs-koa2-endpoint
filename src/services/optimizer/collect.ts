...
const convertToFSRSItem = (datum: ParseData[]): FSRSItem[] => {
  const history = datum
    .map((data) => [
      convertTime(data.review_time, 8, 4),
      parseInt(data.review_rating)
    ])
    .sort((a, b) => a[0] - b[0])
...
function groupReviewsByCard(records: ParseData[]) {
  const reviewsByCard: Record<string, ParseData[]> = {}
  ...
  records.forEach((record) => {
    ...
    reviewsByCard[cardId].push(record)
  })
...
export const analyzeCSV = async (text: Buffer | string) => {
  try {
    ...
    const cardIdHistory = groupReviewsByCard(records)
