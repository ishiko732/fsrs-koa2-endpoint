...
function progress(
  ...
  if (err) {
    console.error(
      `[enableShortTerm=${enableShortTerm}] Progress callback error:`,
      err
    )
    return
  }
  console.log(
    `[enableShortTerm=${enableShortTerm}] progress value`,
    progressValue
  )
}
...
export async function trainTask(
  enableShortTerm: boolean,
  fsrsItems: BasicFSRSItem[]
) {
  const fsrs_items = fsrsItems.map(
    (item: BasicFSRSItem) =>
      new FSRSItem(
        item.map((review) => new FSRSReview(review.rating, review.deltaT))
      )
  )
